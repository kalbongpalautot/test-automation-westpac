...
import nz.govt.msd.driver.web.pagefactory.PageObjectAwareHtmlElementDecorator;
import ru.yandex.qatools.htmlelements.loader.decorator.HtmlElementLocatorFactory;

/**
 * Provides core functionality for pages objects using the page object factory
 * pattern.
 * 
 * @author Andrew Sumner
 *
 * @param <T>
 *            Class of the PageObject extending this class, so that we can
 *            implement fluent style methods
 */
public abstract class BasePageObject<T extends BasePageObject<T>> {
	 ...
	 * @param test
	 *            Calling class
	 * @param timeoutWaitInSeconds
	 *            Maximum time to wait for page to finish loading
	 * @param params
	 *            An optional set of parameters to pass to the waitUntilPageIsLoaded
	 *            method
	 */
	...
	/**
	 * Updates all fields in this class (and descendant classes) with new pointers
	 * to the elements on the webpage we're pointing at.
	 * 
	 * This is useful in the event that an Ajax request has reloaded large portions
	 * of your page and to avoid StaleElementExcpetions it is easier just to refresh
	 * all fields that work out which ones need updating.
	 */
	 ...
	 * 
	 * <ul>
	 * <li>Check that an element on the page that uniquely identifies this page is
	 * present</li>
	 * <li>Ensure that the page is fully loaded (eg Ajax requests have completed) so
	 * don't have to implement waits when accessing elements on the page</li>
	 * </ul>
	 * 
	 * All PageObjects must implement this method.
	 * 
	 * @param timeoutWaitInSeconds
	 *            Number of seconds to wait for the page to load before timing out
	 * @param params
	 *            List of parameters passed to the constructor, if any
	 */
	...
	/**
	 * Provide a reference to the calling class that implements the TestDriveable
	 * interface.
	 * 
	...
	/**
	 * IE will often show a security certificate warning page when accessing a
	 * website, if present select continue to website.
	 * 
	 * @param browser
	 *            Browser
	 */
	protected static void ignoreSecurityCertificateWarning(Browser browser) {
		// For some reason unable to access elements on this page, javascript workaround
		// is required
	 ...
	 * 
	 * <p>
	 * A default description is constructed assuming that a click is about to be
	 * performed on an element in the format: {@literal Clicking '<element text>'}
	 * </p>
	 * 
	 * @param element
	 *            Element to highlight, or null if not applicable
	 * @return A self reference
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * 
	 * @param description
	 *            Description to include with screenshot
	 * @return A self reference
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Notify listener that it should take a screenshot of the current page and then
	 * click the supplied element and return a new instance of the expected page.
	 * 
	 * <p>
	 * A default description is constructed in the format:
	 * {@literal Clicking '<element text>'}
	 * </p>
	 * 
	 * @param <P>
	 *            The type of the desired page object
	 * @param element
	 *            Element to click
	 * @param expectedPage
	 *            Class of page that should be returned
	 ...
	 * @return new PageObject of type expectedPage
	 */
	public <P extends BasePageObject<P>> P capturePageAndSubmit(WebElement element, Class<P> expectedPage) {
		return pageHelper.capturePageAndSubmit(element, expectedPage);
	}
	 ...
	 * @return new PageObject of type expectedPage
	 */
	public <P extends BasePageObject<P>> P capturePageAndClick(WebElement element, String description,
			Class<P> expectedPage) {
	 ...
	 * 
	 * Requires that the expectedPage extends from PageObject and has a public
	 * constructor with a single parameter of TestDriveable.
	 *
	 * @param <P>
	 *            The type of the desired page object
	 * @param expectedPage
	...
	/**
	 * Override this method to implement any code that needs to be run prior to
	 * taking a screen shot, eg fixing floating headers.
	 */
	 ...
	 *            how long to wait for the expected condition to evaluate to true
	...
	/**
	 * Similar to {@link defaultContect} but will always select the main document
	 ...
	 * 
	 * @return Empty string if main document selected otherwise will return name
	 *         property if set, id property if set, else 'UNKNOWN FRAME' for
	 *         selected iframe.
	 */
