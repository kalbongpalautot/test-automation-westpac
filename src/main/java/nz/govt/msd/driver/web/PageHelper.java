...
public class PageHelper {
	 ...
	 * 
	 * @param pageObject
	 *            PageObject this class belongs to
	 */
	...
	/**
	 * Notify and attached listeners that the current page should perform a visual
	 * regression check.
	 * 
	 * @param triggerElement
	 *            Element to check, or null to check the entire window
	 * @param tag
	...
	/**
	 * Capture a screenshot of the current page and add it to the log and story
	 * board.
	 * 
	 * <p>
	 ...
	 *            A custom screenshot taker
	 * @param description
	 *            Description to include with screenshot
	 */
	public void capturePage(ScreenshotTaker screenshotTaker, String description) {
		pageObject.getLogger().with().message(description).screenshot(screenshotTaker)
				.marker(StoryboardMarkerFactory.addCard(pageObject.getSimpleName())).locationAwareParent(this).debug();
	}
	...
	public void capturePage(WebElement element, String description, CardResult result) {
		pageObject.getLogger().with().message(description)
				...
				.locationAwareParent(this).debug();
	}

	/**
	 * Capture a screenshot of the current page and add it to the log and story
	 * board and then click the supplied element and return a new instance of the
	 * expected page.
	 * 
	 * <p>
	...
	public <P extends BasePageObject<P>> P capturePageAndSubmit(WebElement element, Class<P> expectedPage) {
		return capturePageAndSubmit(element, getClickMessage(element), expectedPage);
	}
	 ...
	 * @return new PageObject of type expectedPage
	 */
	public <P extends BasePageObject<P>> P capturePageAndSubmit(WebElement element, String description,
			Class<P> expectedPage) {
		capturePage(element, description);
	...
	public String getCurrentFrameNameOrId() {
		String script = "var frame = window.frameElement;" + "if (!frame) {" + "    return '';" + "}"
				+ "if (frame.name) {" + "    return frame.name;" + "}" + "if (frame.id) {" + "    return frame.id;"
